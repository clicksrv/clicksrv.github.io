...
(function (VideoPlaybackState) {
    VideoPlaybackState[VideoPlaybackState["Playing"] = 0] = "Playing";
    VideoPlaybackState[VideoPlaybackState["Paused"] = 1] = "Paused";
    VideoPlaybackState[VideoPlaybackState["Ended"] = 2] = "Ended";
    VideoPlaybackState[VideoPlaybackState["Error"] = 3] = "Error";
...
class ArDetector {
     ...
     * @param executionTime
     */
    addPerformanceTimeMeasure(performanceObject, executionTime) {
    ...
    getTimings() {
        ...
        for (let i = 0; i < this.performance.drawImage.sampleTime.length; i++) {
            drawWindowTime += (_a = this.performance.drawImage.sampleTime[i]) !== null && _a !== void 0 ? _a : 0;
            getImageDataTime += (_b = this.performance.getImageData.sampleTime[i]) !== null && _b !== void 0 ? _b : 0;
        }
     ...
     */
    async animationFrameBootstrap(timestamp) {
    ...
    async frameCheck() {
        ...
        try {
            ...
            startTime = performance.now();
            try {
                this.context.drawWindow(window, this.canvasDrawWindowHOffset, 0, this.canvas.width, this.canvas.height);
            }
            catch (e) {
    ...
    fastLetterboxPresenceTest(imageData, sampleCols) {
        ...
        for (let i = 0; i < sampleCols.length; ++i) {
            ...
            currentMax = Math.max(imageData[colOffset_r], imageData[colOffset_g], imageData[colOffset_b], 
            ...
            currentMin = Math.min(currentMax, currentMin);
        }
        if (currentMin < this.blackLevel) {
            this.blackLevel = currentMin;
        }
